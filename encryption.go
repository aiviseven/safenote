//copy from github.com/flew-software/filecrypt/v2/encryption.go at 2022-01-04
package main
...
// CreateHashArgon creates a argon2 Hash from the key
func CreateHashArgon(key string, salt []byte) (Hash, error) {
	return Hash(argon2.IDKey([]byte(key), salt, 1, 60*1024, 4, 32)), nil
}

// EncryptSHA256 encrypts byte slice using the passphrase
func EncryptSHA256(data []byte, p string) (EncryptedData, error) {
	...
	key, err := CreateHashArgon(p, salt)
	if err != nil {
		return nil, err
	}
...
// DecryptSHA256 decrypts byte slice using the passphrase
func DecryptSHA256(data []byte, p string) (DecryptedData, error) {
...
// CheckSupport checks if current version of FileCrypt supports the fcef format
